...
func getRetainerItems(character_id string, retainer_id string) []Item {
	...
	retainerItemElements.Each(func(_ int, retainerItemElement *goquery.Selection) {
		...
		items = append(items, Item{Name: name, Quantity: quantity, HighQuality: highQuality, CanBePlacedInArmoire: canBePlacedInArmoire, IsUnique: isUnique, ItemCategory: itemCategory})
	})
	return items
}
